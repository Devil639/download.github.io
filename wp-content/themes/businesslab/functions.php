 ...
 */
function businesslab_theme_setup(){
    /*
     * Make child theme available for translation.
     * Translations can be filed in the /languages/ directory.
     */
    load_child_theme_textdomain( 'businesslab', get_stylesheet_directory() . '/languages' );	
	require get_stylesheet_directory() . '/inc/customizer/businesslab-customizer-options.php';	
}
add_action( 'after_setup_theme', 'businesslab_theme_setup' );

/**
 * Load assets.
 */

function businesslab_theme_css() {
	wp_enqueue_style( 'businesslab-parent-style', get_template_directory_uri() . '/style.css' );
	wp_enqueue_style('businesslab-child-style', get_stylesheet_directory_uri() . '/style.css');
	wp_enqueue_style('businesslab-default-css', get_stylesheet_directory_uri() . "/assets/css/theme-default.css" );
	wp_enqueue_style('businesslab-bootstrap-smartmenus-css', get_stylesheet_directory_uri() . "/assets/css/bootstrap-smartmenus.css" );
}
add_action( 'wp_enqueue_scripts', 'businesslab_theme_css', 99);
 ...
 *
 */
function businesslab_parent_theme_options() {
...
add_action( 'after_switch_theme', 'businesslab_parent_theme_options' );

/**
 * Fresh site activate
 *
 */
$fresh_site_activate = get_option( 'fresh_businesslab_site_activate' );
if ( (bool) $fresh_site_activate === false ) {
	...
	set_theme_mod( 'aasta_theme_color', 'theme-deepskyblue' );
	...
	update_option( 'fresh_businesslab_site_activate', true );
}

/**
 * Custom Theme Script
*/
function businesslab_custom_theme_css() {
	$businesslab_testomonial_background_image = get_theme_mod('aasta_testomonial_background_image');
	?>
    <style type="text/css">
		<?php if($businesslab_testomonial_background_image != null) : ?>
		.theme-testimonial { 
		        background-image: url(<?php echo esc_url( $businesslab_testomonial_background_image ); ?>); 
...
<?php }
add_action('wp_footer','businesslab_custom_theme_css');

/**
 * Page header
 *
 */
function businesslab_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'businesslab_custom_header_args', array(
		'default-image'      => get_stylesheet_directory_uri().'/assets/img/businesslab-page-header.jpg',
		'default-text-color' => 'fff',
		'width'              => 1920,
		'height'             => 500,
		'flex-height'        => true,
		'flex-width'         => true,
		'wp-head-callback'   => 'businesslab_header_style',
	) ) );
}

add_action( 'after_setup_theme', 'businesslab_custom_header_setup' );

/**
 * Custom background
 *
 */
function businesslab_custom_background_setup() {
	add_theme_support( 'custom-background', apply_filters( 'businesslab_custom_background_args', array(
		'default-color' => '202020',
		'default-image' => '',
	) ) );
}
add_action( 'after_setup_theme', 'businesslab_custom_background_setup' );


if ( ! function_exists( 'businesslab_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see aasta_light_custom_header_setup().
	 */
	function businesslab_header_style() {
